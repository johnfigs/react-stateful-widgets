...
export default function Programmers() {
  ...
  const [pioneers, updatePioneers] = useState(listOfAwesome);
  const [activePioneer, updateActivePioneer] = useState(null);
  ...
  const getNameOfFeatured = () => {
    ...
    return pioneers[activePioneer - 1].name;
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: activePioneer ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          pioneers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { updateActivePioneer(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
